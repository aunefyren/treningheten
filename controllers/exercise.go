...
// APIGetExerciseYears returns the distinct years the user has logged exercise days
// in (newest first). It is the cheap read for the exercises listing page, which only
// needs the year list — no day enrichment is done.
func APIGetExerciseYears(context *gin.Context) {
	...
	years, err := database.GetDistinctExerciseYearsForUser(userID)
	if err != nil {
		logger.Log.Info("Failed to get exercise years. Error: " + err.Error())
		context.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to get exercise years."})
		context.Abort()
		return
	}

	context.JSON(http.StatusOK, gin.H{"message": "Exercise years retrieved.", "years": years})
}

// APIGetExerciseDaysSummary returns the slim per-day summary (id, date, note and
// the on-exercise count) for the listing page, optionally filtered to one year. It
// deliberately skips the operation/set/Strava enrichment the page does not display.
func APIGetExerciseDaysSummary(context *gin.Context) {
	...
	var exerciseDays []models.ExerciseDay
	year, hasYear := context.GetQuery("year")
	if hasYear {
		...
		exerciseDays, err = database.GetExerciseDaysForUserUsingUserIDAndYear(userID, yearInt)
		if err != nil {
			logger.Log.Info("Failed to get exercise days. Error: " + err.Error())
			context.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to get exercise."})
			context.Abort()
	...
	dayIDs := make([]uuid.UUID, 0, len(exerciseDays))
	for _, day := range exerciseDays {
		dayIDs = append(dayIDs, day.ID)
	}

	counts, err := database.GetOnExerciseCountsForExerciseDayIDs(dayIDs)
	if err != nil {
		logger.Log.Info("Failed to get exercise counts. Error: " + err.Error())
		context.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to get exercise counts."})
		context.Abort()
		return
	}

	summaries := make([]models.ExerciseDaySummary, 0, len(exerciseDays))
	for _, day := range exerciseDays {
		summaries = append(summaries, models.ExerciseDaySummary{
			ID:               day.ID,
			Date:             day.Date,
			Note:             day.Note,
			ExerciseInterval: counts[day.ID],
		})
	}

	context.JSON(http.StatusOK, gin.H{"message": "Exercise days retrieved.", "exercise": summaries})
}
