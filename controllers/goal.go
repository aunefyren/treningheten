...
func APIRegisterGoalToSeason(context *gin.Context) {
	...
	if err != nil {
		logger.Log.Info("Failed to get season by ID. Error: " + err.Error())
		context.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to get season by ID."})
		context.Abort()
		return
	} else if season == nil {
		logger.Log.Info("Failed to find season by ID.")
	...
	if err != nil {
		...
		logger.Log.Info("User already has a goal for season: " + season.ID.String())
	...
	if err != nil {
		logger.Log.Info("Failed to create goal for season. Error: " + season.ID.String())
...
func ConvertGoalToGoalObject(goal models.Goal) (models.GoalObject, error) {
	...
	if err != nil {
		logger.Log.Info("Failed to process sickleave. Setting to 0.")
...
func ConvertGoalsToGoalObjects(goals []models.Goal) ([]models.GoalObject, error) {
	...
	for _, goal := range goals {
		...
		if err != nil {
			logger.Log.Info("Failed to convert goal to goal object. Returning. Error: " + err.Error())
			return []models.GoalObject{}, err
		}
...
func APIDeleteGoalToSeason(context *gin.Context) {
	...
	if err != nil {
		logger.Log.Info("Failed to get goal by ID. Error: " + err.Error())
	...
	if err != nil {
		logger.Log.Info("Failed to disable goal in database. Error: " + err.Error())
...
func APIGetGoals(context *gin.Context) {
	...
	if err != nil {
		logger.Log.Info("Failed to get goals. Error: " + err.Error())
	...
	if err != nil {
		logger.Log.Info("Failed to convert goals to goal objects. Error: " + err.Error())
